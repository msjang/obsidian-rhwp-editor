...
- HWP/HWPX 뷰 툴바에 `기본 앱에서 열기` 동작을 추가했다. 읽기 모드와 편집 모드에서 모두 사용할 수 있다.
- 현재 HWP/HWPX 파일을 시스템 기본 앱으로 여는 명령 팔레트 동작을 추가했다.

### Fixed

- Obsidian 안에서 편집기 인쇄 미리보기가 팝업으로 차단되던 문제를 수정했다. rHWP의 기존 인쇄 미리보기 흐름을 유지하면서 Obsidian popout view로 표시한다.

### Changed

- 포함된 `rhwp` 런타임을 `0.7.15`로 갱신했다.
